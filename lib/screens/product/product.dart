...
import 'package:kirpa/filters/filter_quantity.dart';
import 'package:kirpa/mixins/mixins.dart';
import 'package:kirpa/product_interaction.dart';
import 'package:kirpa/screens/product/add_on/helper.dart';
import 'package:kirpa/screens/product/add_on/model.dart';
import 'package:kirpa/screens/product/add_on/validate.dart';
import 'package:kirpa/screens/product/widgets/product_featured_image.dart';
...
import 'package:kirpa/models/product/product_type.dart';
import 'package:kirpa/screens/auth/login_screen.dart';
import 'package:kirpa/screens/product/widgets/product_addons.dart';
import 'package:kirpa/service/helpers/request_helper.dart';
import 'package:kirpa/store/app_store.dart';
...
import 'package:kirpa/store/product_recently/product_recently_store.dart';
...
import '../../widgets/product_expiry_details.dart';
    ...
    switch (type) {
      ...
      case ProductBlocks.description:
        return ProductDescription(
          ...
          expand: expand,
          align: align,
          pTitle: translate('product_description'),
          pDetails: productOrVariableProduct?.description ?? '',
        );
      ...
      case ProductBlocks.advancedField:
        ...
        if (customFieldName == "ExpiryDate") {
          var info = getMetaValue(
              key: "woo_expiry_date",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');

          return ProductExpiryDate(pTitle: translate('expiryDate') + info);
        }
        //
        // Removed as its added in the price section now, 21st Jan 2024
        if (customFieldName == "DiscountPriceRemoved" && _authStore.isLogin) {
          var isEnabled = getMetaValue(
              key: "_alg_wc_wholesale_pricing_per_product_enabled",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');

          if (isEnabled == "yes") {
            var totalLevels = getMetaValue(
                key: "_alg_wc_wholesale_pricing_levels_number",
                meta: productOrVariableProduct?.metaData,
                defaultValue: '');

            totalLevels = int.parse(totalLevels);

            var pricesArr = [];
            var quantityArr = [];
            for (var i = 1; i <= totalLevels; i++) {
              var info2 = getMetaValue(
                  key: "_alg_wc_wholesale_pricing_level_discount_$i",
                  meta: productOrVariableProduct?.metaData,
                  defaultValue: '');

              var info3 = getMetaValue(
                  key:
                  "_alg_wc_wholesale_pricing_level_min_qty_$i",
                  meta: productOrVariableProduct?.metaData,
                  defaultValue: '');

              pricesArr.add(info2);
              quantityArr.add(info3);
            ...
            return Container(
              decoration: BoxDecoration(
                border: Border.all(
                    width: 1, color: Color.fromARGB(255, 240, 56, 107)),
                  ...
                  for (var i = 0; i < totalLevels; i++)
                    i == 0
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                              width: 1,
                              color: Color.fromARGB(255, 240, 56, 107)),
                          borderRadius: BorderRadius.only(
                              topLeft: Radius.circular(10),
                              topRight: Radius.circular(10)),
                        ),
                        children: [
                          Text(quantityArr[i] + " pcs.",
                              textScaleFactor: 1,
                        ...
                        TableRow(children: [
                          TableCell(
                            child: Row(
                              ...
                              children: <Widget>[
                                new Text(quantityArr[i] + " pcs."),
                                new Text("€ " + pricesArr[i]),
                              ],
                            ),
                          )
                        ])
        ...
        if (customFieldName == "advicePrice" && _authStore.isLogin) {
          // Advice Price € 3.99 piece
          // Adviesprijs € 3.99 stuk
          var info = getMetaValue(
          ...
          return ProductExpiryDate(
              pTitle: translate('advicePrice') +
                  info +
                  translate('pContentsPeices'));
        } else if (customFieldName == "productsContents") {
          var info = getMetaValue(
              key: "inhoud",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');
          ...
          return ProductExpiryDate(
              pTitle:
              translate('pContents') + info + translate('pContentsPeices'));
        } else if (customFieldName == "upcCode") {
          var info = getMetaValue(
              key: "jk_sku",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');

          return ProductExpiryDate(pTitle: translate('upcCode') + info);
        } else if (customFieldName == "productInfo") {
          var info = getMetaValue(
              key: "_bhww_info_wysiwyg",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');
          ...
          return ProductDescription(
            ...
            expand: expand,
            align: align,
            pTitle: translate('product_info'),
            pDetails: info ?? '',
          );
        } else if (customFieldName == "productNutrition") {
          var info = getMetaValue(
              key: "_bhww_nutrition_wysiwyg",
              meta: productOrVariableProduct?.metaData,
              defaultValue: '');
          ...
          return ProductDescription(
            ...
            expand: expand,
            align: align,
            pTitle: translate('product_nutrition'),
            pDetails: info ?? '',
          );
        } else if (customFieldName == "producstAllergens") {
          /*  var info = getMetaValue(
              key: "_bhww_allergens_wysiwyg",
              meta: productOrVariableProduct?.metaData,
              defaultValue: ''); */

          var info = translate('disclaimer_text');

          return ProductDescription(
            product: productOrVariableProduct,
            expand: expand,
            align: align,
            pTitle: translate('product_allergenen'),
            pDetails: info,
          );
        }

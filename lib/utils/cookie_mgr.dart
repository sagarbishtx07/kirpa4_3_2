...
class CookieManager extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    if (options.uri.queryParameters.containsKey('clear-cookies')) {
      // Delete all cookies and do NOT load the old cookies.
      cookieJar.deleteAll().then((_) {
        options.headers[HttpHeaders.cookieHeader] = null;
        handler.next(options);
      });
    } else if (options.uri.queryParameters.containsKey('bypass-cookies')) {
      // Bypass cookies completely
      handler.next(options);
    } else {
      // Load cookies even if there is an Authorization header (to merge cart)

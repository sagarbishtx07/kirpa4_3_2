...
import 'package:kirpa/models/address/country.dart';
import 'package:kirpa/models/message/message.dart';
import 'package:kirpa/models/models.dart';
import 'package:kirpa/models/order_note/order_node.dart';
import 'package:kirpa/models/post_author/post_author.dart';
import 'package:kirpa/models/product/attributes.dart';
import 'package:kirpa/models/product/product_prices.dart';
import 'package:kirpa/models/product/product_variable.dart';
import 'package:kirpa/models/product_review/product_review.dart';
import 'package:kirpa/screens/press/press.dart';
import 'package:kirpa/service/constants/endpoints.dart';
import 'package:kirpa/service/modules/network_module.dart';
import 'package:kirpa/utils/utils.dart';
import 'package:kirpa/mixins/mixins.dart' as mixins;
...
class RequestHelper {
  ...
  Future<Map<String, dynamic>> login({Map<String, dynamic>? queryParameters}) async {
    try {
      ...
      log("DioError in login: ${e.message}");
      if (e.response != null) {
        log("login: Response data: ${e.response?.data}");
        log("login: Response status code: ${e.response?.statusCode}");
        log("login: Response headers: ${e.response?.headers}");
      }
      log("login: Request options: ${e.requestOptions}\n ${e.error} \n $e");
      rethrow;
    } catch (e, stacktrace) {
      // Catch and log any other unexpected errors
      log("login: Unexpected error: $e");
      log("login: Stacktrace: $stacktrace");
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> selectShipping({
    ...
    try {
      ...
      print("selectShipping $res");
      return res;
    }on DioException catch (e) {
      // Log the DioError details
      log("DioError in getCart: ${e.message}");
      if (e.response != null) {
        log("selectShipping: Response data: ${e.response?.data}");
        log("selectShipping: Response status code: ${e.response?.statusCode}");
        log("selectShipping: Response headers: ${e.response?.headers}");
      }
      // Log any additional error information, such as the request and response if available
      log("selectShipping: Request options: ${e.requestOptions}");
      rethrow;
    } catch (e, stacktrace) {
      // Catch and log any other unexpected errors
      log("selectShipping: Unexpected error in getCart: $e");
      log("selectShipping: Stacktrace: $stacktrace");
      rethrow;
    }
  }
  ...
  Future<dynamic> checkout({Map<String, dynamic>? queryParameters, dynamic data, Map<String, dynamic>? header}) async {
    try {
      ...
      log("DioError in checkout: ${e.message}");
      if (e.response != null) {
        log("checkout: Response data: ${e.response?.data}");
        log("checkout: Response status code: ${e.response?.statusCode}");
        log("checkout: Response headers: ${e.response?.headers}");
      }
      log("checkout: Request options: ${e.requestOptions}\n ${e.error} \n $e");
      rethrow;
    } catch (e, stacktrace) {
      // Catch and log any other unexpected errors
      log("checkout: Unexpected error: $e");
      log("checkout: Stacktrace: $stacktrace");
      rethrow;
    }
  }
